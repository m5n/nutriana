-- =========================================================================================================
-- USDA National Nutrient Database for Standard Reference, Release 26 (http://www.ars.usda.gov/ba/bhnrc/ndl)
...
create table NUTR_DEF (
    ...
    Tagname varchar(20),   -- International Network of Food Data Systems (INFOODS) Tagnames. (INFOODS, 2012.) A unique abbreviation for a nutrient/food component developed by INFOODS to aid in the interchange of data.
...
-- Assert all 632894 records were loaded
create table tmp (c int unique key);
insert into tmp (c) values (2);
insert into tmp (select count(*) from NUT_DATA);
delete from tmp where c = 632894;
insert into tmp (select count(*) from tmp);
drop table tmp;
...
;
-- Assert all 15137 records were loaded
create table tmp (c int unique key);
insert into tmp (c) values (2);
insert into tmp (select count(*) from WEIGHT);
delete from tmp where c = 15137;
insert into tmp (select count(*) from tmp);
drop table tmp;
...
;
-- Assert all 541 records were loaded
create table tmp (c int unique key);
insert into tmp (c) values (2);
insert into tmp (select count(*) from FOOTNOTE);
delete from tmp where c = 541;
insert into tmp (select count(*) from tmp);
drop table tmp;
...
load data local infile './usda_nndsr/data/LANGUAL.txt'
    ...
    (NDB_No, Factor_Code)
;
-- Assert all 38804 records were loaded
create table tmp (c int unique key);
insert into tmp (c) values (2);
insert into tmp (select count(*) from LANGUAL);
delete from tmp where c = 38804;
insert into tmp (select count(*) from tmp);
drop table tmp;
...
;
-- Assert all 570 records were loaded
create table tmp (c int unique key);
insert into tmp (c) values (2);
insert into tmp (select count(*) from DATA_SRC);
delete from tmp where c = 570;
insert into tmp (select count(*) from tmp);
drop table tmp;
...
;
-- Assert all 213653 records were loaded
create table tmp (c int unique key);
insert into tmp (c) values (2);
insert into tmp (select count(*) from DATSRCLN);
delete from tmp where c = 213653;
insert into tmp (select count(*) from tmp);
drop table tmp;
